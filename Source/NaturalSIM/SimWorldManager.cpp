...
void ASimWorldManager::UpdateActiveRegions() {
	...
	for (FIntPoint Key : CachedChunkKeys) {
		...
		Chunk.bIsActiveRegion = Chunk.bGeomorphologyDirty || Chunk.FaultStress > 50.0f || Chunk.PendingVisualFlags != 0;
	}

	float CWorldSize = (ChunkSize - 1) * 50.0f;
	auto MarkActive = [&](FVector2D Pos) {
		int32 CX = FMath::FloorToInt(Pos.X / CWorldSize);
		int32 CY = FMath::FloorToInt(Pos.Y / CWorldSize);
		FIntPoint Key(CX, CY);
		if (FChunkData* Chunk = WorldChunks.Find(Key)) Chunk->bIsActiveRegion = true;
		};

	if (HumanModule) {
		for (const FTribeData& Tribe : HumanModule->Tribes) {
			if (Tribe.Population > 0) MarkActive(Tribe.Position);
		}
	}
	if (FaunaModule) {
		for (const FAnimalData& Animal : FaunaModule->Animals) {
			if (Animal.HerdSize > 0) MarkActive(Animal.Position);
		}
	}

	for (FIntPoint Key : CachedChunkKeys) {
		if (WorldChunks[Key].bIsActiveRegion) ActiveChunkKeys.Add(Key);
